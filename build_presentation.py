...
def build():
    ...
    _content_slide(prs, "2. Обоснование и новизна", [
        ("Прежняя версия: только «болен/здоров» по клеткам мозга.", 0),
        ("Минусы: нет промежуточной стадии; биопсию мозга живому не сделать.", 1),
        ("Решение: экспрессия генов в КРОВИ — у живого пациента, есть все 3 стадии.", 0),
        ("Сравнила ~10 аналогов: по отдельности всё есть, вместе — ни у кого.", 0),
        ("Новизна = соединение: гены крови + 3 стадии + минимальная панель +", 0),
        ("интерпретируемость + рабочий сайт + честная валидация (открытые данные).", 1),
    ])
    ...
    _content_slide(prs, "3. Реферат — функционал", [
        ...
        (f"Ожидаемый результат: честный macro-AUC ≈ {m['macro']:.2f}.", 0),
    ])
    ...
    _content_slide(prs, "4. Функциональная схема", [
        ...
        ("Честная CV: отбор и масштабирование ВНУТРИ фолдов (без утечки).", 1),
    ...
    _content_slide(prs, "6. Структура проекта", [
        ...
        ("biomarker_panel.py — ЯДРО: отбор панели, честная CV, модель, SHAP.", 0),
    ...
    _content_slide(prs, "8. Результаты", [
        ...
        (f"Честный macro-AUC = {m['macro']:.2f} "
         f"(95% ДИ {m['ci'][0]:.2f}–{m['ci'][1]:.2f}).", 0),
        (f"AUC по стадиям: норма {m['ctl']:.2f}, MCI {m['mci']:.2f}, "
         f"деменция {m['ad']:.2f}.", 0),
        ("Норму от болезни отделяет уверенно; MCI — труднее (честно показано).", 1),
        ("Утечки/переобучения нет (наивный ≈ честный отбор).", 0),
    ])
    ...
    _content_slide(prs, "Заключение", [
        ...
        ("Честная методология: метрики по классам, доверительные интервалы.", 0),
