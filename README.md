...
Датасеты, требующие институциональной аффилиации (университет / больница), — ADNI, OASIS-3, UK Biobank — недоступны для независимого исследователя. Реалистичные альтернативы, доступные бесплатно прямо сейчас:

| Датасет | Размер | Где взять |
|---|---|---|
| **Alzheimer's Dataset (4 класса МРТ)** | 6 400 снимков | kaggle.com/datasets/tourist55/alzheimers-dataset-4-class-of-images |
| **MRI and Alzheimer's** | 150 пациентов | kaggle.com/datasets/jboysen/mri-and-alzheimers |
| OASIS-1/2 (уже в проекте) | 586 визитов | — |

**Alzheimer's Dataset (4 класса)** — реальные МРТ-срезы, размеченные по степени тяжести (NonDemented / VeryMild / Mild / Moderate). 6 400 снимков — в 10 раз больше текущего датасета. Под него можно добавить CNN-ветку: сырой МРТ-срез → свёрточный энкодер → Gated Fusion вместо трёх FreeSurfer-чисел.

```
Текущий пайплайн:    FreeSurfer (3 числа) → MRIEncoder (MLP)
Расширенный:         МРТ-срез (224×224)   → CNN-энкодер → Gated Fusion
```
...
**3. CNN на МРТ-срезах (доступно уже сейчас)**

Kaggle-датасет с 6 400 снимками позволяет перейти от трёх FreeSurfer-чисел к сырым МРТ-срезам. Для 2D-срезов достаточно стандартного ResNet-18/EfficientNet без GPU-кластера.

```
Текущий пайплайн:    FreeSurfer → 3 числа → MLP
Следующий шаг:       МРТ-срез (224×224) → ResNet → эмбеддинг → GatedFusion
```

Полные 3D-снимки (NIfTI) с 3D-CNN достигают AUC 0.92–0.96 (Spasov et al., 2019), но требуют ADNI/OASIS-3 и GPU.
