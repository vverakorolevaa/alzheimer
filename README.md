...
> **Новизна (коротко):** интерпретируемый веб-прототип стадийной диагностики болезни Альцгеймера по минимальной панели генов крови с честной по-классовой валидацией и персональным SHAP-объяснением; не претендует на клиническую сертификацию.
...
**Незанятая ниша = наша новизна:** соединить в одном инструменте всё сразу — *гены крови + 3 стадии + минимальная панель + интерпретируемость + развёрнутый сайт + честная валидация на открытых данных*. По отдельности это есть у разных проектов, вместе — нет.

Сильная сторона — **честность**: мы приводим метрики по каждой стадии и прямо показываем, что MCI распознаётся труднее всего, а не прячем это за общим числом.
...
- **Ожидаемый результат:** честный `macro ROC-AUC ≈ 0.75`; уверенное отделение нормы от болезни (AUC 0.86), MCI — труднее (AUC 0.66).
...
biomarker_panel.py ── ОТБОР ПАНЕЛИ + МОДЕЛЬ (новизна) ───────────────┐
   ...
   │  честная стратифицированная CV + бутстрэп 95% ДИ                 │
   ▼                                                                  ▼
...
Честная стратифицированная кросс-валидация (отбор генов и масштабирование — **строго внутри обучающих фолдов**):
...
**Контроль утечки:** «наивный» отбор генов на всех данных даёт macro-AUC 0.74 ≈ честные 0.75 → **переобучения/утечки нет**.
...
├── biomarker_panel.py  — ★ отбор панели + 3-классовая модель + честная CV + SHAP
...
- Varoquaux G. (2018). Cross-validation failure: small sample sizes lead to large error bars. *NeuroImage*, 180, 68–77. — почему важны доверительные интервалы и честная CV.
